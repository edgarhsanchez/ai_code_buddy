...
async fn perform_code_analysis(source_branch: &str, target_branch: &str, repo_path: &str) -> anyhow::Result<Review> {
    ...
    // Analyze changes
    let (metrics, changed_files) = git_analyzer.analyze_changes_between_branches(source_branch, target_branch)?;
    review.metrics = metrics;
    
    // Detect technology stack
    review.technology_stack = git_analyzer.detect_technology_stack(&changed_files)?;
    
    println!("🔬 Performing code analysis on {} files...", changed_files.len());
    ...
    for file_path in &changed_files {
        if should_analyze_file(file_path, &config) {
            ...
            if let Ok(content) = git_analyzer.get_file_content_at_commit(
                file_path, 
                source_branch
            ) {
    ...
    review.overall_assessment = ai_assessment.unwrap_or_else(|e| {
        format!("AI assessment unavailable: {}", e)
    });
...
fn should_analyze_file(file_path: &str, config: &ReviewConfig) -> bool {
    ...
    let include_match = config.include_patterns.iter().any(|pattern| {
        match pattern.as_str() {
            ...
            "*.cpp" | "*.cc" | "*.cxx" => file_path.ends_with(".cpp") || file_path.ends_with(".cc") || file_path.ends_with(".cxx"),
...
fn print_cli_summary(review: &Review) {
    ...
    println!("📝 Commits analyzed: {}", review.branch_comparison.commits_analyzed.len());
    
    let critical = review.get_critical_issues().len();
    let high = review.get_high_priority_issues().len() - critical;
    
    if critical > 0 {
        println!("🚨 Critical issues: {} (REQUIRES IMMEDIATE ATTENTION)", critical);
    }
    if high > 0 {
    ...
    for (category, issues) in &review.issues {
        if !issues.is_empty() {
            ...
            for (i, issue) in issues.iter().take(3).enumerate() {
                ...
                println!("    {} {} {}", severity_icon, issue.file_path, 
