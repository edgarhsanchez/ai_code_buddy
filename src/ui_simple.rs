...
impl AppState {
    ...
    pub fn generate_review_report(&self) -> String {
        ...
        let mut category_issues: std::collections::HashMap<&IssueCategory, Vec<&CodeIssue>> = std::collections::HashMap::new();
        for issue in &self.all_issues {
            category_issues.entry(&issue.category).or_insert_with(Vec::new).push(issue);
        }
        ...
        // Technology Stack
        if !self.review.technology_stack.programming_languages.is_empty() {
            report.push_str("## Technology Stack\n\n");
            report.push_str(&format!("**Languages:** {}\n", 
                self.review.technology_stack.programming_languages.join(", ")));
...
fn print_overview(state: &AppState) {
    ...
    println!("📝 Commits: {}", state.review.branch_comparison.commits_analyzed.len());
...
fn print_issues_list(state: &AppState) {
    ...
    let start = if state.selected_issue_index >= 5 { state.selected_issue_index - 5 } else { 0 };
    ...
    for i in start..end {
        if let Some(&issue_idx) = state.filtered_issues.get(i) {
            if let Some(issue) = state.all_issues.get(issue_idx) {
                let marker = if i == state.selected_issue_index { "► " } else { "  " };
                let severity_icon = match issue.severity {
                    Severity::Critical => "🔴",
                    Severity::High => "🟠",
                    Severity::Medium => "🟡",
    ...
    if state.filtered_issues.len() > 10 {
        println!("\n... showing {} of {} issues", end - start, state.filtered_issues.len());
    }
...
fn generate_summary_report(state: &AppState) -> String {
    format!("# Code Review Executive Summary\n\n\
        **Review Date:** {}\n\
        **Branches:** {} → {}\n\
        **Files Changed:** {}\n\
        **Total Issues:** {}\n\
        ...
        **High Priority Issues:** {}\n\n\
        ...
        if state.all_issues.iter().any(|i| matches!(i.severity, Severity::Critical)) {
            "🚨 **DO NOT MERGE** - Critical issues require immediate attention."
        } else if state.all_issues.iter().any(|i| matches!(i.severity, Severity::High)) {
...
fn generate_critical_issues_report(state: &AppState) -> String {
    ...
    if critical_issues.is_empty() {
        ...
        report.push_str(&format!("⚠️ **ACTION REQUIRED** - {} critical/high-priority issues found:\n\n", critical_issues.len()));
        ...
        for (i, issue) in critical_issues.iter().enumerate() {
            ...
            report.push_str(&format!("## {}{} Issue {}: {:?}\n\n", severity_icon, severity_icon, i + 1, issue.category));
